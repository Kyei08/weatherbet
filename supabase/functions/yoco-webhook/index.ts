...
const verifyYocoSignature = async (payload: string, signature: string, secret: string): Promise<boolean> => {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!(await verifyYocoSignature(rawBody, signature, webhookSecret))) {
